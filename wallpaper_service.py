...
def load_config():
    if not os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, "w") as f:
            json.dump({"change_interval": DEFAULT_INTERVAL}, f)
    with open(CONFIG_FILE, "r") as f:
        config = json.load(f)
    return config.get("change_interval", DEFAULT_INTERVAL)

def save_config(interval):
    with open(CONFIG_FILE, "w") as f:
        json.dump({"change_interval": interval}, f)
...
def main():
    ...
    schedule.every(interval).seconds.do(change_wallpaper)
    logging.info(f"Wallpaper changer running every {interval} seconds")
    while True:
